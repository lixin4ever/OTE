...
def lstm_extractor(train_set, test_set, embeddings, win_size=1):
    ...
    LSTM_extractor.add(MyEmbedding(output_dim=dim_symbol,
                                   ...
                                   mask_zero=True, input_length=max_len*win_size, winsize=win_size))
    #LSTM_extractor.add(Reshape((max_len, win_size * dim_symbol)))
    LSTM_extractor.add(Bidirectional(LSTM(100, return_sequences=True), merge_mode='concat', input_shape=(max_len, dim_symbol * win_size)))
